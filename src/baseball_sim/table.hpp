...
class Table_Row {
    public:
        ...
        std::string get_stat(std::string stat_name, std::string default_val) const ;
        int get_stat(std::string stat_name, int default_val) const ;
        float get_stat(std::string stat_name, float default_val) const ;

        bool has_stat(std::string stat_name) const {
            return row_data.find(stat_name) != row_data.end();
        }

        bool has_attributes(std::map<std::string, std::vector<std::string>> attributes) const ;
        std::map<std::string, std::string> get_row_data() const {
            return row_data;
        }

    private:
        std::string get_raw_stat_data(std::string stat_name) const {
...
class Stat_Table {
    public:
        ...
        template <class T>
        T get_stat(std::string stat_name, unsigned int row_index, T default_val) const {
