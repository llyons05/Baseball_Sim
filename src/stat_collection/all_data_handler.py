...
def scrape_and_save_player_data(player_page_url: str, player_id: str, stat_types: list[DI.PLAYER_STAT_TYPES], year: int, overwrite_data: bool = True) -> bool:
    client = Scraping_Client()
    if utils.BASE_URL not in player_page_url:
        player_page_url = utils.BASE_URL + player_page_url

    for stat_type in stat_types:
        if (not DI.is_player_file_up_to_date(player_id, stat_type, year)) or overwrite_data:
